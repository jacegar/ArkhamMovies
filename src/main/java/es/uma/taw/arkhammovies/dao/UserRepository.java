...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("select u.id, u.nickname, size(u.reviews) from User u order by size(u.reviews) desc")
    List<Object[]> getSortedUserReviews();

    @Query("select u.id, u.nickname, size(u.moviesLiked) from User u order by size(u.moviesLiked) desc")
